...
const contactSchema = z.object({
  firstName: z
    ...
    .regex(/^[a-zA-ZÀ-ÿ\u00C0-\u017F\s\-'\.]+$/, "Le prénom contient des caractères non autorisés")
    .refine(name => {
  ...
  lastName: z
    ...
    .regex(/^[a-zA-ZÀ-ÿ\u00C0-\u017F\s\-'\.]+$/, "Le nom contient des caractères non autorisés")
    .refine(name => {
      return !FORBIDDEN_NAME_PATTERNS.some(pattern => pattern.test(name));
  ...
  website: z.string().optional().refine(val => !val || val.length === 0, "Champ non autorisé"),
});
